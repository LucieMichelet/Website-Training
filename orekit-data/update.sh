...
#  MSAFE/mmm####f10-prd.txt (where mmm is a month abbreviation and #### a year)
...
msafe_url=https://www.nasa.gov/wp-content/uploads/2019/04
...
while [ ! -z "$msafe_base" ] ; do
    ...
    if $(cd MSAFE ; fetch_URL ${msafe_url}/${msafe_file}) ; then
      ...
      echo 1>&2 "${msafe_file} not published yet"
      test -e MSAFE/${msafe_file} && rm MSAFE/${msafe_file}
      msafe_base=""
    fi
done
